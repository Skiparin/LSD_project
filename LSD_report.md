...
Bagefter havde vi brug for frameworks til både frontend og backend delen af vores system, og her havde at vi mest brugt Flask som et API framework, men også en smule til frontend. Vi begyndte at undersøge, om det var muligt at bruge Flask til vores system. Problemet med Flask “out of the box” er, at det er single threaded og ikke kan holde på så mange requests af gangen som så mange andre frameworks. Men efter nærmere undersøgelse, fandt vi ud af, at ved at bruge uWSGI, kunne vi godt bruge Flask. uWSGI er en måde at hoste flere Python applikationer til NGINX, lighttpd, og cherokee mellem andre. Ved brug af uWSGI laver vi så et socket, som NGINX kan bruge.
#
### Software design
Når vi overvejede hvordan vi skulle designe vores software, var ydeevne og dataintegritet punkterne vi ville have mest fokus på. Så et af vores første spørgsmål var, i hvilket lag vi skulle have meget af logikken. Vi kom frem til, at siden logikken er relativt simpel og da Python er et dynamisk sprog og derfor er relativt langsomt, ville vi have noget af logikken i databasen med triggers. Med brug af triggers kommer vi til at lave færre kald frem og tilbage til databasen, og da triggers er transaction secure, så har vi en del dataintegritet. 
 
Vi havde overvejet, om vi skulle bruge Psycopg2 i stedet for SQLAlchemy, da SQLAlchemy er et Python library bygget ovenpå Psycopg2 for at give ORM og andre muligheder som Psycopg2 ikke har. Men da det er bygget ovenpå og giver ekstra funktionalitet, så er det langsommere end Psycopg2. Så det var en overvejelse at skifte library, hvis der var en stor ydelsesforskel.
...
I starten af forløbet, virkede deres statusside heller ikke korrekt, idet den ikke viste de rigtige statusser for systemets services. Det var derfor svært for os som operators, at se tidspunktet hvor de ville crashe deres eget system, og vi kunne på den måde ikke notificere dem, da vi i starten ikke vidste at siden viste forkert. Vi kontaktede dem derefter omkring problemet, hvor vi så fik at vide, at alt kørte, men at statussiden ikke viste det korrekt. Og da vi heller ikke havde adgang til deres Grafana, kunne vi heller ikke derigennem tjekke, om deres services faktisk var gået ned eller ej. Dette fik de dog hurtigt ordnet, så vi på statussiden kunne holde øje med hvornår deres system ville gå ned.
...
Det gik godt i starten, men efterhånden som vi kom længere ind i processen, endte vores operator job desværre med at blive et punkt, som vi ikke fik gjort så meget ud af. Specielt, kunne vi sagtens have lavet issues på deres GitHub vedrørende deres hjemmesides performance. Udover det, var det også begrænset hvor meget kommunikation, der var fra den anden gruppe, da deres Grafana side i store dele af projektet ikke virkede ordentligt. Dete betød at vi aldrig fik et Grafana endpoint fra dem, som samtidig gjorde det meget svært for os, at undersøge om deres system overholdte vores SLA kriterier. Da gruppen dog endelig havde fået Grafana til at virke, fik vi det tilset på deres computer, hvor alt så helt fint ud og umiddelbart overholdte vores SLA.
...
I vores projekt er vi kommet rundt om en masse forskellige emner og problemer, som har været både i starten og henimod slutningen af projektet. Vi synes hele projektet som helhed, har været et sundt forhold, hvor vi har lært en masse forskellige teknologier at kende. Da projektet har været meget stort, synes vi dog der har været dele af den der har skulle været lidt nedprioriteret i forhold til andre, det har hovedsageligt været i forhold til vores frontend. Her har vi ikke gjort det helt store ud af det, da vi ikke har følt denne del, var nær så vigtig baseret på de requirements, der var lagt op til os. Derudover har vi heller ikke gjort nok ud af lave CI testing, dvs. hver gang vi deployer, har vi ikke har nogen tests.

Systemet kunne have været lavet bedre med hensyn til load balancing, og hvordan vi gemmer vores requests. Tanken var at bruge Nginx til at fordele requesting ud til uWSGI og uWSGI ville så holde på de requests indtil en af dens workers kunne bearbejde dem. Planen var også, at NGINX og uWSGI ikke skulle genstartes, fordi vi bruger et uWSGI værktøj, der hedder auto-reload. Auto-reload kigger på om filerne i Flask applikationen er blevet ændret, og ville så reloade workers og på den måde ville systemet køre uden at skulle lukkes. Men det virker ikke altid sådan som man har planlagt. og vi skulle nogen gange genstarten på grund af problemer med uWSGI.

Den anden del af projektet betod af at vi har skulle monitorere på en anden gruppes projekt. Dette var en fin opgave, men også noget som vi fik aldrig rigtig fik arbejdet helt i bund med, da vi ikke fik lavet issues, eller en anden form for dokumentation, som vi måske skulle burde have gjort. Vi fik dog derimod snakket lidt med den anden gruppe, hvor vi fik et fint indblik i, hvordan deres program fungerede. Det var dog ikke det mest spændende, især da vi havde en masse andre projekter kørende samtidig. Vi kan dog godt se pointen i, hvorfor vi skulle prøve sådan en opgave, da det giver en bedre mulighed for i fremtiden at kunne analysere og se, hvordan en anden gruppe har håndteret nogle af de samme problemer, som vi har haft. Operating har givet en god mulighed for at få et indblik i hvordan man kan smoke teste, hvilket i firmaer stadig er en essentiel ting at skulle kunne og meget brugbart. Helt generelt har det været godt at se projektet i et andet perspektiv, end det man normalt vil have, så vi kan lave forbedringer i vores tilgang til vores fremtidige projekter.

#
### Group work reflection & lessons learned
Den første ting som vi mener, vi kunne have forbedret, er i forhold til vores forløb som operators. Vi brugte ikke forfærdeligt meget tid på at monitorere gruppens projekt, og vi skulle havde været bedre til at bede om adgang til deres Grafana. Dette kunne vi f.eks. let havde gjort gennem et GitHub issue.

En anden ting, er at vi godt kunne have smidt mere tid ind i vores arbejdsprocess. Vi skulle have været bedre opretholde vores fokus på hvor var i projektet, og tale om de problemer, som man støtte på. Selvfølgelig hjalp vi hinanden, men bedre intern kommunikation kunne helt sikkert have gavnet projektets resultat.

Endelig, kunne vores investering af tid i design perioden godt have været bedre. Vi brugte tid på at lave en meget overfladisk design plan, som vi så endte med at bruge, i stedet for at lave en mere dybdegående plan. Da vi så senere fik mere specifikke design opbygningskrav, blev vi nemlig nødt til at redesign dele, for at det kunne virke med resten af systemet.
